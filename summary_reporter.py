...
def main():
    print("📊 Compiling Master Session Ledger...")
    
    # GitHub VM par reports dhoondo
    report_files = glob.glob("NodeReport_*.json")
    
    if not report_files:
        print("⚠️ No Node Reports found! Local run mein ye error normal hai.")
        return

    master_csv = "Master_Upload_Ledger.csv"
            ...
            try:
                with open(rep_file, 'r') as rf:
                    ...
                    writer.writerow([
                        data.get("timestamp", ""),
                        data.get("node", ""),
                        data.get("batch_folder", ""),
                        data.get("primary_assigned_acc", ""),
                        data.get("final_uploaded_acc", ""),
                        data.get("status", ""),
                        data.get("files_count", 0)
                    ])
                print(f"✅ Merged {rep_file}")
            except Exception as e:
                print(f"❌ Error reading {rep_file}: {e}")

    print(f"🎉 Master Ledger '{master_csv}' updated!")

    # 🚀 AUTO-PUSH TO REPO (Sirf GitHub Actions par kaam karega)
    if os.environ.get("GITHUB_ACTIONS"):
        try:
            subprocess.run(["git", "config", "--global", "user.name", "github-actions"], check=True)
            subprocess.run(["git", "config", "--global", "user.email", "github-actions@github.com"], check=True)
            subprocess.run(["git", "add", master_csv], check=True)
            subprocess.run(["git", "commit", "-m", "📊 Update Master Ledger [skip ci]"], check=True)
